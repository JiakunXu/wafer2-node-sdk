...
function saveUserInfo (userInfo, skey, session_key) {
    ...
    const union_id = userInfo.unionId
    ...
    .then(res => {
        ...
        if (res[0].hasUser) {
            return mysql('cSessionInfo').update({
                uuid, skey, create_time, last_visit_time, session_key, user_info, union_id
            }).where({
                open_id
            })
        } else {
            return mysql('cSessionInfo').insert({
                uuid, skey, create_time, last_visit_time, open_id, session_key, user_info, union_id
            })
        }
    })
    .then(() => ({
